...
```
Table\@ref(tab:table1) demonstrates the baseline chracteristics of the cleaned survey data from 2019CES, which hase the reduced amounts of variables. It shows the total 23445 observations of the data without the missing values. It shows the proportion of the each subcategory's observations among the total observation. For example, it presents that 13.3% of people pose that they are somewhat likley to vote for the 2019 election. This similar interpretation can apply to all the variable's subcategory.
...
This is the model of voting Liberal:
$$
...
This is the model of voting Conservative:
$$
...
```{r, table2, echo = FALSE}
#Results(post-stractification) vote for liberal
...
kable(aic_summary, align = "c", format = "markdown", col.names = c("Model Name", "AIC"),
      caption = "Summary - AIC Score of final Models")
```
...
kable(broom::tidy(survey_model), align = "c", format = "markdown", digits = 3,
      ...
      caption = "Summary for the Survey Logistic Regression Model of voting Liberal")
```
...
kable(broom::tidy(survey_model2), align = "c", format = "markdown", digits = 3,
      col.names = c("Term", "Estimate", "Standard Error", "Test Statistic", "P-Value"),
      caption = "Summary for the Survey Logistic Regression Model of voting Conservative")
```
...
$$
`survey_model2`is the model that are finally selected in predicting the proportion of people voting for the COnservative Pary in 2019 Canadian Federal Election. Based on the summary table for the model, we can determine the estimated values of the intercept, $\hat{\beta_0}$ and estimated slopes $\hat{\beta_i}$ (for i = 1 to 17). Therefore, here is the fitted logistic regression for voting Conservative.
...
hist_edu <- ggplot(new_survey, aes(as.character(vote_liberal), fill = edu_level)) + geom_bar(stat = "count") +
  theme_bw() + labs(x = "Vote for liberal or not", y = "Number of People", fill = "edu_level") + theme(legend.text = element_text(size=5))

hist_edu_con <- ggplot(new_survey, aes(as.character(vote_conservative), fill = edu_level)) + geom_bar(stat = "count") + theme_bw() + labs(x = "Vote for liberal or not", y = "Number of People", fill = "edu_level") + theme(legend.text = element_text(size=5))
